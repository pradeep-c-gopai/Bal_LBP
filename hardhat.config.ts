...
task('deploy', 'Run deployment task')
  ...
  .setAction(
    async (args: { id: string; force?: boolean; key?: string; verbose?: boolean }, hre: HardhatRuntimeEnvironment) => {
      ...
      // console.log('HERE DEPLOY', hre.network, apiKey)
      console.log("hre.network", hre.network.name, hre.network.provider, hre.network.config);
      console.log('apikey: ', apiKey);
      const verifier = apiKey ? new Verifier(hre.network, apiKey) : undefined;
      console.log("verifier: ", verifier)
      console.log("hre.config.networks[hre.network.name] ", hre.config.networks[hre.network.name])
      console.log('(hre.config.networks[hre.network.name] as any).verificationAPIKey ', (hre.config.networks[hre.network.name] as any).verificationAPIKey)
...
export default {
  ...
  networks: {
    baseSepolia: {
      url: process.env.SEPOLIA_URL || "https://sepolia.base.org",
      accounts: [`0x${process.env.PRIVATE_KEY}`],
      chainId: 84532,
      verificationAPIKey : '2DQT4IGJ1QJ6VCHY7TC4798NUSVBWVE37I'
    },
  },
  etherscan: {
    customChains: [
      ...
      },
      {
        ...
        apiKey: "m5TCE23SINZCSNUA9RVG88VW4XY86ZJN9ZX",
        verificationAPIKey: "m5TCE23SINZCSNUA9RVG88VW4XY86ZJN9ZX",
      },
      {
        network: 'fraxtal',
        chainId: 252,
